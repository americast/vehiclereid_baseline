...
def train(train_loader, model, criterion,optimizer, epoch):
    ...
    for i, (image, target, camid, image_2, target_2, camid_2, result) in enumerate(train_loader):
        ...
        image_2 = torch.autograd.Variable(image_2, volatile=True).cuda()
        output, _ = model((image, image_2))
        
        loss = criterion(output, result)

        prec1= accuracy(output, result, topk=(1,2 ))
        losses.update(loss.item(), image.size(0))
        top1.update(prec1[0], image.size(0))
        top2.update(prec1[1], image.size(0))
